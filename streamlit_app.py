...
WORKFLOW_TASK = "Professional Tasks — Prompt + Rubric"
WORKFLOW_ENV = "Duck Environments — Environment Creation"

workflow = st.radio(
    "Which part of the program are you checking?",
    options=[WORKFLOW_TASK, WORKFLOW_ENV],
    horizontal=True,
    help=(
    ...
    with st.expander("Environment checks", expanded=True):
        ...
        with e2:
            env_depth_tokens = st.number_input(
                "Minimum environment depth indicator",
                min_value=1_000,
                value=256_000,
                step=10_000,
                help=(
                    "An environment below 256k total source tokens cannot support a "
                    "256k+ downstream task. Passing this indicator alone does not prove "
                    "that a future prompt genuinely requires 256k."
                ),
            )
    ...
    if "env_estimates" in st.session_state:
        ...
        if env_estimates:
            ...
            m2.metric(
                ...
                f"{total_source:,}",
                (
                    f"≥ {int(env_depth_tokens):,} depth indicator"
                    if depth_met
                    else f"{int(env_depth_tokens) - total_source:,} short"
                ),
            )
            m3.metric(
                "Distinct formats",
            ...
            if file_band_met and depth_met and formats_met:
                st.success(
                    ...
                    "256k depth indicator, and format-diversity check are satisfied."
                )
            else:
                missing = []
                ...
                if not depth_met:
                    missing.append("256k depth indicator")
                if not formats_met:
            ...
            if total_source > 1_000_000:
                st.info(
                    f"Environment context is ~{total_source:,} tokens. That is allowed in "
                    "**Environment Creation**: no 1M upper warning is applied here because "
                    "the full environment is not the same thing as the context required by "
                    "one downstream Prompt + Rubric task."
                )
            else:
                st.info(
                    "No upper token limit is applied in Environment Creation mode. "
                    "The goal here is a dense, usable source environment rather than keeping "
                    "the entire environment below the Prompt + Rubric tasking ceiling."
                )
            ...
            env_json = {
                ...
                "checks": {
                    ...
                    "depth_indicator_tokens": int(env_depth_tokens),
                    "depth_indicator_met": depth_met,
...
**Professional Tasks — Prompt + Rubric**  
...
**Duck Environments — Environment Creation**  
Checks the **source environment** before downstream tasks are written. Environment size follows
the assigned file-count band, needs **3+ formats**, and must be dense enough to support **256k+**
tasks. The app intentionally applies **no upper token ceiling** in this mode.
